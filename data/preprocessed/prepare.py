...
FILES = [
    ...
    {'filename': 'admin/baltic_a0_expanded', 'post_sql': 'insert into vectiles_input.baltic_a0_expanded (left_name, right_name, left_country_code, right_country_code, on_water, geom) select (properties->>\'left_name\')::varchar as left_name, (properties->>\'right_name\')::varchar as right_name, (properties->>\'left_country_code\')::varchar as left_country_code, (properties->>\'right_country_code\')::varchar as right_country_code, (properties->>\'on_water\')::boolean as for_road, st_force2d(st_transform(st_setsrid(st_geomfromgeojson(geometry::text), 4326), 3301)) as geom from (select datas#>\'{properties}\' as properties, datas#>\'{geometry}\' as geometry from data_upload) f'},
]
